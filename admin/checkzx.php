...
if ($username != NULL and $password != NULL and $yzm != NULL) {
    ...
    if (strtolower($yzm) != $_SESSION["yzm"]) {
        echo "<script>alert('验证码不正确，请重新输入！');location.href='zc.php';</script>";
    } else {
        // 检查用户名是否已存在
        ...
        if (mysqli_num_rows($check_username_result) > 0) {
            ...
            // 注册成功，跳转到登录界面
            echo "<script>alert('注册成功，请登录！');location.href='login.php?username={$username}&password={$password}';</script>";
        }
    }
} else {
